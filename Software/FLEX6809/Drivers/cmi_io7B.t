...
m_einch equ $F806 with ech0
m_outch equ $F80A
m_tstat equ $F808
m_start equ $F800
m_vector equ $FFE0
cable equ io_space+$07EE
...
* needs cmi_sbug version 1.6
* works with PC-cable and Straigth-cable.
*
* Added step rate and double step for each drive.
*
* Cleanup 12-10-2025 CdeJ
*
* Last modified 15-12-2025 CAJ
* Speedup FLP driver part

 lib flpinfo.h
dr_shift set $E400 always split CMI_IO7B !!!!
...
**************************************
 ...
 JMP Init also set DP
 JMP SetDP get DP from monitor
 JMP SEEK
 FDB 0 driver table
...
************************************
retry rzb 1
ltcopy rzb 1 keep for correct version nr.
...
*
**************************************
SEEK pshs d,DP save calling regs
 ldaa drvDP
 tfr A,DP
 ldaa ,s track
 jsr setdsk
...
chkr02 lda <fo2cmd
 ...
 bra chkr01
*
chkr03 puls b reset stack
*
chkr01 puls d,DP,y,pc
...
sts02 ldb curdrv set lower 4 bits for drive select!
 ldx #drvtab Straight cable drive table
 abx entry
 ora 0,x get drive bits
 sta <fo4lat select the drive
 rts
...
*
 ...
 sta <fo2cmd
*
b01 orcc #$50 disable ints
 bra f03
* loop here
b02 lda <fo2dat get data
 if (WD2791=1)
 coma
 endif
 sta 0,u+ transfer
...
f03 lda <fo4sta
 ...
 bne f03
 decb at zero dec B
 bne f03 if zero abort
 bra f98
*
f97 ldb <fo2cmd read status
 if (WD2791=1)
 comb
 endif
*
...
f98 lda #FD_FI0+8 force immediate interrupt
 if (WD2791=1)
 coma
 endif
 sta <fo2cmd

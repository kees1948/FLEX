...
* Last modified 18-12-2025 CAJ
* Speedup FLP driver part
* modified 18-11-2024 CAJ
...
* $E600-$E6FF IDE driver
  ...
  lib flpinfo.h
dr_shift set $E400 always split CMI_IDE7 !!!!
...
* The CPUXXCMI has the IDE driver at $E200
...
* Define a Default drive descriptor so drivers work
...
* gobal fopr all drives
 org $DE42
retry fcb $00
ltcopy fcb $00 keep for correct version nr.
lside fcb $00
ldens fcb $00
...
* next 4 items have fixed place
drvtrk fcb 0,0,0,0
drvsec fcb 0,0,0,0
drvstp fcb 0,0,0,0 step rate each drive
drvtab fcb $5,$A,$1,$2

drvflg fcb 0 cable type
...
* see if the drive number is an IDE partition
WINSET pshs x 
 ldx DRVPTR 
 TST DISK,x 
 puls X,PC
...
* do the IDE stuff here - it was done in REST1
 PULS DP,X,PC
 
drv01 ldaa 3,X get drive number
 tst >drvflg PC cable?
 beq drv04
...
drv04 JSR GETPHYS get the physical drive number in B
 CMPB #3 max 4 drives
 BLS drv02 
...
* save drive info previously selected floppy drive.
drv02 ldab drvDP
 tfr B,DP
...
*
 ...
 sec set the carry bit in the CCR
 BRA chkr01 return
...
fndtrk LDX #drvtrk
 ...
 ORG dr_shift always !!!!
 fdb DRVBEG mark driver version
*
...
dsktyp  equ     *
        fcb     MX5TRK0,MX5SDS0,0,$40
DTYPSZ  equ     *-dsktyp
        fcb     MX5TRK0,MX5DDS0,1,$40
        fcb     MX8TRK0,MX8SDS0,0,$00
        fcb     MX8TRK0,MX8DDS0,1,$00
...
sts02 PSHS A save A - has bits for latch
 ldaa >curdrv
...
asmDP ldx #drvtab Change drvtab bits
 LDAB [C_cable] check boot cable
 CMPB #5 PC cable?
 BNE stl05 yes - do straight thru cable
 lda #%00000101 set drive 0 on PC cable $05
 ldb #%00001010 set drive 1 on PC cable $0A
 std 0,x
 std 2,x
 staa >drvflg set PC cable!
 rts all pointers set
stl05 ldd #%0000000100000010
 std 0,x drive 0, drive 1
 ldd #%0000010000001000
 std 2,x drive 2, drive 3
 clr >drvflg set straight cable!
 rts all pointers set
...
frdblk PSHS DP,X,U save the direct page register
 ...
 TFR B,DP register now points to the FDC
 LDAB #31  very long   create timeout
 LDX #0            65536*2*25/4 cycles
 STAA <fo2cmd set the command in FDC
 ORCC #$50 set the interrupt flag = no interrupts
 BRA frb03 

frb02 LDAA <fo2dat get byte from FDC data register
 STAA ,U+ 
...
*
* for driver purpose, the IDE device is put in LBA mode.
*
* Drive arangemnet rewrite is for 16MB partitions
* You may format a smaller size, 
* but the driver works in 16MB blocks
* IDE/SD devices use 512 byte sector size 
* and 16 bit transfer per read.
*
* Offset is the number of 16MB offsets 
* for the partition being accessed.
*
* DRIVE DESCCRIPTORS - WINCHESTER
*
DISK equ $0 0=FLOPPY <>0 - WINCHESTER
DRVNO equ DISK+1 HARDWARE DRIVE NR - Must be 0 or 1
STEP equ DRVNO+1 Not used was STEP RATE
INITED equ STEP+1 0=NOT INITIALIZED
SHIFT equ INITED+1 Not used was <9 NR OF SHIFTS >8 DIV
HMASK equ SHIFT+1 Not used was HEAD MASK
OFFSET equ HMASK+1 Partition OFFSET - nr of 16MB offsets
...
BASADR EQU io_space+$0180 CPU09IDE
...
* DISK JUMP TABLE
*
* This is here to make the IDE driver portable,
* do not remove
* If dr_split = $E400 then read $E6.. 
* for the following entry's

READ JMP READ1 $E200 <- READ is not referenced anywhere
...
rslop ldd <STATUS READ STATUS (actual status in B)
 aslb check for DRQ
...
wrsect bsr TSKSET
 ...
 clr 0,-s init bytecount on stack
*
wrlop ldd <STATUS READ STATUS (actual status in B)
 aslb check for DRQ
...
*************************************************
*
* INPUT D = 16 BIT LOGICAL SECTOR NR WITHIN THE PARTITION
* 
...
* the logical sector number is set,
* now add in the partition offset
...
* While there is nothing to do for a modern drive, 
* FLEX expects DRIVE SELECT to be called by this routine. 
* It is therefore necessary to set the DRVPTR pointer.
...
* DRIVE READY TEST 
*
* Drive number found in the FCB must be checked for ready
*
* Entry
*     X - FCB address - 3,X contains drive number
* Exit
...
* COLD START - INIT
*
* Do any driver initialization here on the system boot.
*
* Entry
*     No parameters are given
* Exit

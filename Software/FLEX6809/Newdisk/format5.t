 TTL 6809 FORMAT5 PROGRAM
 PAG
* FORMAT5
*
* DISK FORMATTING PROGRAM FOR 6809 FLEX
* 5" format S/D side S/D density
*
* Copyright (c) 1984 by
...
* TABLES.  THIS VERSION IS SETUP FOR AN 5 INCH DISK.
*
...
SMAXS1 EQU 20 SD MAX SIDE 1 SECTORS
* DOUBLE DENSITY:
DMAXS0 EQU 18 DD MAX SIDE 0 SECTORS
DMAXS1 EQU 36 DD MAX SIDE 1 SECTORS
**************************************************
...
Logical RMB 1 Drive number
Physical rmb 1 Drive number
SIDE RMB 1
DBSDF RMB 1
DENSE RMB 1
DNSITY RMB 1
...
INFO bra INFEND
 ...
 fcc   'Can use cmisbug6 and IDE driver'
INFEND
...
FORM1 LDA #SMAXS0 INITIALIZE SECTOR MAX
 ...
 lib sysconf.t define iospace and DP
 lib flpdata.h define FDC

 lda lcable
 cmpa #1 straight cable?
 beq tst3
PCcable CMPB #1 ENSURE 0 TO 1 PC cable
 LBHI EXIT
 bra tstdone
...
DBOOT1 ldb lcable get cable type
 beq Orit
 cmpb #1
 beq DOIT auto boot loader
 cmpb #5
 bne DOIT auto boot loader
Orit ora #%00000101  drive select 1
...
WPST FCC 'DISK IS PROTECTED!'
 FCB 4
SCRDS FCC '5" SCRATCH DISK IN DRIVE '
 FCB 4
...
SECST FCC 'TOTAL SECTORS = '
 FCB 4
msg80 fcc 'FORMAT 80 TRACKS? '
 fcb 4
msg40 fcc 'FORMAT 40 TRACKS? '
 fcb 4
msg35 fcc 'FORMAT 35 TRACKS? '
 fcb 4
...
WRTTRK  pshs    dp,d,x,u
        lda     ldp get real DP
        setdp   fdcDP =$E1 or $F1 no problem
        tfr     a,dp
        lda     #%01000000      set 5", FDC speed
        tst     DNSITY
        bne     f10
        ora     #LA_SDN         set single
f10     tst     SIDE
        beq     f11
        ora     #LA_SID
...
PCtab   ldx     #DRBTPC      No
        bra     TABPC
TAB4    ldx     #DRBTAB      Yes
TABPC   ldb     Physical     drive number
        abx
        ora     0,x          get drive select bit
        sta     <fo4lat
*
        ldx     #0
        clra
...
BOOT    equ     *
*
* bootsector code for FLEX on a CPU09CMI system
*
*
...
*
fdccmd  equ     fdcbas+00
fdctrk  equ     fdcbas+01
fdcsec  equ     fdcbas+02
fdcdat  equ     fdcbas+03
fdcsel  equ     fdcbas+04
fdcsta  equ     fdcbas+08
*
* sbug routines
pdata   equ    $f812
monitr  equ    $f800
...
boots   lda #fdcDP Do Not change the boot loader
        setdp fdcDP
        bra    boot2

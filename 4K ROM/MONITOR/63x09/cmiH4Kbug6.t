...
romsize set 4 for 2K rom, use 4 for the 4K rom
version set 6
* Set same value's below in the NAM statement
 NAM CMI_BUG 4.6  cpu09cmi monitor
...
* Show CPU type and ROM size on first page
 if cpu=6809&io_space=$E000
 STTL Motorola M68x09 - 4K rom
 endif
 if cpu=6309&io_space=$E000
 STTL HITACHI HD63x09 - 4K rom
 endif
 if cpu=6809&io_space=$F000
 STTL  Motorola M68x09 - 2K rom
 endif
 if cpu=6309&io_space=$F000
 STTL HITACHI HD63x09 - 2K rom
 endif

* Show correct cmi_bug prompt
Sbugmsg MACRO
 fcc '&1'
 fcb $20
 if cpu=6809
...
*
* change source to asmb 6309 & 6809 monitor (2025-02-16) CAJ
* cmiHbug6 for HD63x09
* cmiMbug6 for M68x09
* conform settings on TOP page.

*
*       *** commands ***
* control a   = alter the "a" accumulator
* control b   = alter the "b" accumulator
* control e   = alter the "e" accumulator 6309 cpu only
* control f   = alter the "f" accumulator 6309 cpu only
...
altrpc lbsr prtpc print msg " pc = "
 ...
 stx 10+W_reg,u poke in new contents @<cpu>
altpcd rts
...
jmptab equ *
 ...
 fdb xbkpnt 
tabend equ *
* ** 63x09 / 68x09 vector addresses **
* following are the addresses of the vector routines
* for the 63x09 / 68x09 processor. during initialization
* they are relocated to ram from $f7c0 to $f7cf. they are
* they are relocated to ram so that the user may revector
* to his own routines if he so desires.
...
start equ *
 ...
 tfr a,dp
 IF cpu=6309 THEN
 fcb $11,$3d,$03 set 63x09 native mode
 ENDIF
*
